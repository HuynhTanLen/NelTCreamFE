...
const getProfileLink = () => {
  if (!state.user) {
      return '/login'; // Chuyển hướng đến trang đăng nhập nếu chưa có user
  }

  // Kiểm tra các vai trò của người dùng
  const roles = state.user.roles.map(role => role.name);
  ...
  // Nếu không phải admin hay seller, mặc định là user
  return '/user/profile';
};
